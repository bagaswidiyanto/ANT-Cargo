...
class Prices extends MY_Controller
{
  ...
  public function index()
  {
    ...
    if ($this->input->post('dari') == '' || $this->input->post('tujuan') == '') {
      ...
      // WHERE mp.custID='1' AND mp.Origin = '".$this->input->post('dari')."' AND mp.Destination = '".$this->input->post('tujuan')."'")->result();
      ...
      $this->data['asal'] = $this->db->get_where('log_kabupaten a', array('a.ID' => $this->input->post('dari')))->row();

      $this->data['tujuan'] = $this->db->get_where('log_kabupaten b', array('b.ID' => $this->input->post('tujuan')))->row();
